...
RUN /opt/R/current/bin/R -q -e 'pak::pak("r-wasm/rwasm#63", lib = .Library)'
...
RUN CRANLIBS=$(curl https://r-universe.dev/api/sysdeps/resolute?stream=1 | jq --slurp -r '.[].packages | flatten[]' | grep -v "libnode") &&\
